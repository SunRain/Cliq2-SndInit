package com.sunrain.sndinitcliq2;
...
public class MainActivity extends Activity implements View.OnClickListener {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		menu.add(0, OPTION_ABOUT, 0, getString(R.string.about));
		menu.add(0, OPTION_CHANGELOG, 0, getString(R.string.changelog));
		menu.add(0, OPTION_INSTRUCTIONS, 0, getString(R.string.instructions));
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case OPTION_ABOUT:
			...
			alertbuilder.setMessage(R.string.AboutInfo);
		...
		case OPTION_CHANGELOG:
			alertbuilder.setTitle(R.string.changelog);
			alertbuilder.setMessage(R.string.ChangeLogInfo);
			alertbuilder.setPositiveButton("Ok",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
								int whichButton) {
		...
		case OPTION_INSTRUCTIONS:
			alertbuilder.setTitle(R.string.instructions);
			alertbuilder.setMessage(R.string.InstructionsInfo);
			alertbuilder.setPositiveButton("Ok",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
								int whichButton) {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tv1.setText(R.string.version + result);

		tv1 = (TextView) findViewById(R.id.tv_already_installed);

		if (ShellInterface.isBootmenuInstalled()) {
			tv1.setText(R.string.SndInitHadInstalled);
		} else {
			tv1.setTag(R.string.SndInitNotInstalled);
		}
	}
	...
	public void onClick(View view) {
		...
		case R.id.btn_install_sndinit:
			...
			/*final Button stableBM = (Button) diag
					.findViewById(R.id.btn_stable_bootmenu);*/
			final Button devBM = (Button) diag
					.findViewById(R.id.btn_dev_bootmenu);
			final Button ownBM = (Button) diag
					.findViewById(R.id.btn_own_bootmenu);
			...
			/*stableBM.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					new DownloadFileTask()
							.execute("http://dl.dropbox.com/u/668793/bootmenu/bootmenu.v0.6.1.tar.gz");
					diag.dismiss();
				}
			});*/
			...
			devBM.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					pdiag.show();
					new DownloadFileTask()
							.execute(Constants.DOWNLOAD_URL);
					diag.dismiss();
				}
			});
			...
			ownBM.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					final Button own_url_btn = (Button) diag_custom_url
							.findViewById(R.id.btn_custom_url_ok);
					final EditText own_url_et = (EditText) diag_custom_url
							.findViewById(R.id.et_custom_url);
					...
					own_url_btn.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View v) {
							...
							pdiag.setMessage(getString(R.string.install_after_download));
							pdiag.show();
							new DownloadFileTask().execute(own_url_et.getText()
									.toString());
							diag.dismiss();
						}
					});
	...
	boolean installRecovery() {
		...
		if (Settings.System.getInt(getContentResolver(),
			...
			if (ShellInterface.isSuAvailable()) {
				...
				command.append(busybox + " cp " + busybox
						+ " /system/xbin/busybox ; ");
				command.append(busybox + " cp " + busybox
						+ " /system/bin/busybox ; ");
				...
				command.append(busybox + " cp " + bootmenu + " /bootmenu/"
						+ Constants.DOWNLOADED_BM + " ; ");
				...
				command.append(busybox + " tar xvpf /bootmenu/"
						+ Constants.DOWNLOADED_BM + " -C / ; ");
				...
				// do backups
				command.append(busybox
						+ " cp /system/bin/bootmenu /system/backups ; ");
				command.append(busybox
						+ " cp /system/bin/logwrapper /system/backups ; ");
				command.append(busybox
						+ " cp /system/bin/logwrapper.bin /system/backups ; ");
				...
				command.append(busybox + " rm /bootmenu/"
						+ Constants.DOWNLOADED_BM + " ; ");

				try {
					// Log.v("SndInit",command.toString());
					ShellInterface.runSuCommand(MainActivity.this,
							command.toString());
				...
				Toast.makeText(
						MainActivity.this,
						"Couldn't obtain ROOT permissions. Is Superuser installed?",
			...
			Toast.makeText(
					MainActivity.this,
					"USB debugging option is disabled. Please go to your phone settings and enable it before proceeding with install.",
					Toast.LENGTH_LONG).show();
			return false;
		}
	}
	...
	private class DownloadFileTask extends AsyncTask<String, Integer, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			...
			return dh.getFileFromURL(urlToDown, getFilesDir()
					.getAbsolutePath() + "/" + Constants.DOWNLOADED_BM);
		}
		...
		protected void onPostExecute(Boolean result) {
			...
			if (result) // if download didn't got an error
				...
				Toast.makeText(MainActivity.this, "Download error. Try again.",
						Toast.LENGTH_LONG).show();
		}
	}
	...
	boolean uninstallRecovery() {
		if (!ShellInterface.isBootmenuInstalled()) {
			Toast.makeText(
					MainActivity.this,
					"BootMenu was not installed already, so there's nothing else to do.",
					Toast.LENGTH_LONG).show();
			return true;
			...
			if (ShellInterface.areBackupFilesPresent()) {
				...
				// " cp /system/bin/bootmenu /system/backups ; ");
				command.append(busybox
						+ " cp /system/backups/logwrapper /system/bin ; ");
				command.append(busybox
						+ " cp /system/backups/logwrapper.bin /system/bin ; ");
				command.append(busybox + " chmod 755 /system/bin/bootmenu ; ");
				command.append(busybox
						+ " chmod 755 /system/bin/logwrapper.bin ; ");
			}
			...
			if (partialUninstall == false)
				Toast.makeText(MainActivity.this, "Uninstalled with success.",
						Toast.LENGTH_LONG).show();
			else
				Toast.makeText(
						MainActivity.this,
						"Uninstalled with success, but only possible to do partial Uninstall.",
						Toast.LENGTH_LONG).show();
			return true;
		} else {
			Toast.makeText(
			...
			try {
				...
				while (entries.hasMoreElements()) {
					...
					Runtime.getRuntime().exec(
							"chmod 777 " + outputFile.getAbsolutePath());
				}
				unzippedAssets = true;
			} catch (IOException e) {
	...
	void copyStreams(InputStream is, FileOutputStream fos) {
		...
		try {
			...
			try {
				...
				Utils.debugFunc("Exception while closing the stream: " + e2,
						Log.ERROR);
			}
		}
	}
